...
export default async function SlingshotMenuGroupEditPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = await params;

    // Safety check - handle invalid UUID if needed, but Next.js usually 404s or error boundary catches
    const { group, allCollections } = await getData(id, 'slingshot');
