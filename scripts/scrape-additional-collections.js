...
async function processCollection(client, slug) {
    const targetUrl = `${BASE_URL}/en-eu/collections/${slug}`;
    console.log(`\n--- Starting sync for: ${slug} ---`);
    ...
    try {
        ...
        const title = doc.querySelector('h1')?.textContent?.trim() || slug.replace(/-/g, ' ');
        ...
        const existingColl = await client.query('SELECT id FROM collections WHERE slug = $1', [slug]);
        let collectionId;
        ...
        for (const prodUrl of productLinks) {
            const prodSlug = prodUrl.split('/products/')[1];
            // console.log(`Processing product: ${prodSlug}`);

            // Check if product exists
            const existingRes = await client.query('SELECT id FROM products WHERE slug = $1', [prodSlug]);
            let productId;
            ...
            if (existingRes.rows.length > 0) {
                ...
                console.log(`-> Product missing: ${prodSlug}. Scraping details...`);
                ...
                try {
                    ...
                    const pTitle = pDoc.querySelector('h1')?.textContent?.trim() || prodSlug;
                    ...
                    const insertProdRes = await client.query(`
                        INSERT INTO products (name, slug, handle, description_html, status, created_at, updated_at)
                        VALUES ($1, $2, $3, $4, 'active', NOW(), NOW())
                        RETURNING id
                    `, [pTitle, prodSlug, prodSlug, pDesc]);
                    ...
                    if (pImage) {
                        try {
                            ...
                            // Ignore image insert dupes
                            console.log('Image insert warning', e.message);
                        }
                    }
                    console.log(`-> Created new product: ${pTitle}`);

                } catch (e) {
                    console.error(`Error scraping product ${prodSlug}:`, e.message);
                    continue;
                }
            }
        ...
        console.error(`Error processing collection ${slug}:`, err);
    }
}
...
async function run() {
    ...
    try {
        for (const slug of TARGET_SLUGS) {
            await processCollection(client, slug);
        }
