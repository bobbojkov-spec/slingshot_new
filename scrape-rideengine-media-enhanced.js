...
/**
 * Rideengine Hero Media Scraper - Enhanced Version
 * 
 * Scrapes all hero images and videos from rideengine.com
 * and organizes them into folders by page source.
 * 
 * Enhanced with Shopify-specific patterns:
 * - Slideshow sections (.slideshow, .slideshow__slide)
 * - Image overlay sections (.image-overlay)
 * - Video components (video-component, section-video)
 * - Responsive images (srcset, data-src)
 * - Height-full/fixed sections
 */
...
// Helper: Extract hero media from HTML (Enhanced for Shopify)
function extractHeroMedia(html, pageUrl) {
    ...
    // Shopify-specific hero section patterns (based on browser inspection)
    const heroPatterns = [
        // Shopify slideshow sections
        /<section[^>]*class="[^"]*slideshow[^"]*"[^>]*>[\s\S]*?<\/section>/gi,
        /<div[^>]*class="[^"]*slideshow[^"]*"[^>]*>[\s\S]*?<\/div>/gi,
        // Image overlay sections
        /<section[^>]*class="[^"]*image-overlay[^"]*"[^>]*>[\s\S]*?<\/section>/gi,
        /<div[^>]*class="[^"]*image-overlay[^"]*"[^>]*>[\s\S]*?<\/div>/gi,
        // Video sections
        /<section[^>]*class="[^"]*section-video[^"]*"[^>]*>[\s\S]*?<\/section>/gi,
        /<video-component[^>]*>[\s\S]*?<\/video-component>/gi,
        ...
        // Height-full sections (common on Shopify)
        /<section[^>]*class="[^"]*height--full[^"]*"[^>]*>[\s\S]*?<\/section>/gi,
        /<section[^>]*class="[^"]*height--fixed[^"]*"[^>]*>[\s\S]*?<\/section>/gi,
    ];
    ...
    heroSections.forEach(section => {
        ...
        // Image tags with data-src (lazy loading)
        const dataSrcMatches = section.matchAll(/<img[^>]*data-src=["']([^"']+)["'][^>]*>/gi);
        ...
        // Image tags with srcset (responsive images)
        const srcsetMatches = section.matchAll(/<img[^>]*srcset=["']([^"']+)["'][^>]*>/gi);
        ...
        // Shopify video-component custom elements
        const videoComponentMatches = section.matchAll(/<video-component[^>]*>[\s\S]*?<\/video-component>/gi);
        for (const videoComp of videoComponentMatches) {
            const videoTags = videoComp[0].matchAll(/<video[^>]*src=["']([^"']+)["'][^>]*>/gi);
            for (const videoTag of videoTags) {
                let videoUrl = videoTag[1];
                videoUrl = new URL(videoUrl, pageUrl).href;
                media.videos.push(videoUrl);
            }
        }
...
// Main execution
async function main() {
    console.log('=== RIDEENGINE HERO MEDIA SCRAPER (Enhanced) ===');
