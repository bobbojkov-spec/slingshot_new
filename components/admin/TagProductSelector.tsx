...
export default function TagProductSelector({ tagEn, onClose, onSave }: TagProductSelectorProps) {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4 sm:p-6 lg:p-8">
            <div className="bg-white rounded shadow-2xl w-[95%] max-w-[1400px] h-[95%] max-h-[900px] flex flex-col overflow-hidden border border-gray-100">
                {/* Header */}
                <div className="flex items-center justify-between p-6 border-b bg-white">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900">Manage Tag Products: {tagEn}</h2>
                        <p className="text-sm text-gray-500 mt-2">Select products to associate with this tag</p>
                    </div>
                    ...
                    <div className="w-1/4 border-r p-4 overflow-y-auto bg-gray-50">
                        ...
                        <div className="space-y-2">
                            {allProducts.filter(p => selectedIds.has(p.id)).map(product => (
                                <div key={product.id} className="flex items-center gap-2 p-2 bg-white rounded border border-gray-100 shadow-sm">
                        ...
                        <div className="p-4 border-b bg-white flex gap-4">
                            <div className="flex-1 relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none"
                                />
                            </div>
                            <select
                                value={selectedBrand}
                                onChange={(e) => setSelectedBrand(e.target.value)}
                                className="px-4 py-2 border border-gray-200 rounded text-sm bg-white outline-none"
                            >
                                ...
                                {filteredProducts.map(product => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`flex gap-4 p-3 rounded cursor-pointer transition-all border ${isSelected ? 'bg-blue-50 border-blue-200 ring-1 ring-blue-100' : 'bg-white border-gray-200 hover:border-blue-300'
                                                }`}
                                        >
                ...
                <div className="p-6 border-t bg-white flex items-center justify-between">
                    ...
                    <div className="flex gap-4">
                        <button onClick={onClose} className="px-6 py-2 border border-gray-300 rounded hover:bg-gray-50 text-sm font-medium">Cancel</button>
                        <button onClick={handleSave} disabled={saving} className="px-8 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50 text-sm font-medium shadow-sm">
