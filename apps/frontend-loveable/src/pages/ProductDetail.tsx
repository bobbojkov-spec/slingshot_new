...
// Multiple images for the gallery (using same image for demo, in real app these would be different angles)
const productImages = [rpxKite, ghostKite, ufoKite];
...
const product = {
  ...
  category: "Kite",
  price: 1899,
  description: "The RPX V2 delivers unmatched performance for freeride and big air. Built with premium materials and refined aerodynamics for explosive power and smooth handling.",
  sizes: ["7m", "9m", "10m", "12m"],
  specs: [
...
const related = [
  { id: "2", name: "Ghost V3", category: "Kite", price: 1799, image: ghostKite, slug: "ghost-v3" },
        ...
        <div className="section-container section-padding">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-16">
            ...
            <div className="flex flex-col justify-center animate-fade-in" style={{ animationDelay: '100ms' }}>
              ...
              {/* Size Selector */}
              <div className="mb-6">
                <span className="font-heading font-semibold text-sm uppercase tracking-wide text-foreground mb-3 block">{t.size}</span>
                <div className="flex gap-2 flex-wrap">
                  {product.sizes.map((size) => (
                    <button key={size} onClick={() => setSelectedSize(size)}
                      className={`px-4 py-2 rounded border font-body text-sm transition-all ${selectedSize === size ? "border-accent bg-accent text-white" : "border-border hover:border-primary"}`}>
                      {size}
                    </button>
                  ))}
                </div>
              ...
              {/* Quantity */}
              <div className="mb-8">
                <span className="font-heading font-semibold text-sm uppercase tracking-wide text-foreground mb-3 block">{t.quantity}</span>
                <div className="flex items-center gap-3">
                  <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="touch-target flex items-center justify-center border border-border rounded hover:border-primary">
                    <Minus className="w-4 h-4" />
                  </button>
                  <span className="font-body text-lg w-12 text-center">{quantity}</span>
                  <button onClick={() => setQuantity(quantity + 1)} className="touch-target flex items-center justify-center border border-border rounded hover:border-primary">
